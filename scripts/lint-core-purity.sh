...
# плюс сам роутер `CLAUDE.md.jinja` — 6 стек-токенов, которых никто не проверял (нашло
# независимое ревью 13.08). Языковые слои (`rules/lang/`, `lang-packs/`) СЮДА НЕ ВХОДЯТ:
# стек-токен там и есть их работа.
CORE_GLOBS="${CORE_PURITY_GLOBS:-${REPO_ROOT}/skeleton/.claude/rules/common/*.md ${REPO_ROOT}/skeleton/.claude/skills/*/SKILL.md ${REPO_ROOT}/skeleton/.claude/docs/*.md.template ${REPO_ROOT}/skeleton/CLAUDE.md.jinja ${REPO_ROOT}/skeleton/PACKAGE_CLAUDE.md.jinja ${REPO_ROOT}/skeleton/docs/specs/_template.md}"
