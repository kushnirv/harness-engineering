...
> ⚠️ **Дефолты анализаторов ниже — из отчёта, НЕ из прогона.** SDK на машине, где писался слой,
> отсутствовал; проверить сборкой не удалось. Первое, что делает проект на .NET: подтверждает или
> опровергает это `dotnet build` на пустом шаблоне. Расхождение — правь этот файл, а не подгоняй
> `.editorconfig` под текст.
>
...
- **Порядок middleware:** ExceptionHandler (+HSTS, не в Development) → `UseHttpsRedirection` →
  ...
  **Точный триггер не проверен прогоном** (нет SDK): на каком именно порядке middleware
  анализатор срабатывает, взято из отчёта. Остальные ошибки порядка не ловит ничто, кроме ревью —
  это часть верна независимо от триггера.
